...
def start(exchanges, historical_data_done_event):
    with sqlite3.connect('funding_data/funding_db/funding.db') as conn:
        ...
        for exchange in exchanges:
            ...
            end_date = int(datetime.strptime('2050-01-30', '%Y-%m-%d').timestamp() * 1_000)
            ...
            for coin in all_coins:
                ...
                for interval in intervals:
                    ...
                    insert_candles(conn, exchange_id, coin_id, interval_id, response, interval)
                    time.sleep(2)
    historical_data_done_event.set()

def insert_candles(conn, exchange_id, coin_id, interval_id, candles, interval):
    cursor = conn.cursor()
    candles_to_insert = []
    for candle in candles['history']:
        candles_to_insert.append((
            ...
            coin_id,
            interval_id,
            int(candle['t']),  # Start time as integer
    ...
    try:
        cursor.executemany('''
            ...
            (exchange_id, coin_id, interval_id, s_t, s, i, o, c, h, l) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        ''', candles_to_insert)
        conn.commit()
