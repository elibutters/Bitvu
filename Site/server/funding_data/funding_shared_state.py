...
import copy
import time
from funding_data.funding_aggregator import update_funding_db

class FundingSharedState:
    ...
    def sync_local_funding_candles(self, local_candles: dict, instruments, exchange):
        last_local_candles = {}
        with self._lock:
...
def start_funding_update_loop(shared_state):
    def update_loop():
        while True:
            try:
                shared_state.update_funding_db()
                time.sleep(0.5)
